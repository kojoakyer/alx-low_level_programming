 ...
 * @min: Lower bound the search range
 * @max: Upper bound  the search range
...
int binary_search_mod(int *array, int min, int max, int value)
{
	...
	if (min <= max)
	{
		...
		/* Check if the middle element the target value */
		if (array[mid] > value)
	...
	return (-1); /* Return -1 if the value not found in the array */
}

/**
 * exponential_search - Exponential search algorithm
 * @array: Pointer the first element of the array to search in
 * @size: Number of elements in array
 * @value: Value to search for
 * Return: Index where value  located, or -1 if not present
 */
...
int exponential_search(int *array, size_t size, int value)
{
	...
	{
		/* Check if the current index out of bounds or the value is found */
		if (i >= (int)size || array[i] >= value)
		{
			/* adjust the index the last valid position and fix range */
			if (i >= (int)size)
				i = size - 1;
