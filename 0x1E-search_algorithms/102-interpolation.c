...
int interpolation_search(int *array, size_t size, int value)
{
	...
	while (low <= high && value >= array[low] && value <= array[high])
	{
		...
		/* Check if value  found at the calculated position */
		if (array[pos] == value)
			return (pos); /* Return the index if the value is found */

		/* Adjust the search range based  the comparison with the value */
		if (array[pos] < value)
			low = pos + 1;
		else
			high = pos - 1;
	...
	/* Print a message if the checked position out range */
