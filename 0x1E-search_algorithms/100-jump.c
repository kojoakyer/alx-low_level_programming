...
int jump_search(int *array, size_t size, int value)
{
	...
	if (array == NULL || size == 0) /* Check if the array  NULL */
		return (-1);
	/* Calculate the block size using square root  size */
	block = sqrt((int)size);
	block_size = block;
	...
	{
		/* Break if the value is found  the current block or initial index */
	...
	while (holder <= block)
	{
		...
		if (array[holder] == value) /*Return the index if the value  found*/
			return (holder);
		holder++; /* Move  the next element in the range */
	}
